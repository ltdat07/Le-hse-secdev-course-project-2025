...
def test_crud_flow():
    ...
    # create tag
    r = client.post("/api/v1/tags", headers=headers, json={"name": "algorithms"})
    assert r.status_code == 200

    # create note
    n = {"title": "Dijkstra", "body": "Shortest path", "tags": ["algorithms", "graphs"]}
    ...
    # list by tag
    r = client.get("/api/v1/notes", headers=headers, params={"tag": "algorithms"})
    assert r.status_code == 200
    assert len(r.json()) >= 1
